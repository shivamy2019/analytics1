...
plot(women, type='l')
plot(women, type='b')
plot(women, type='b', pch=18, lty=2, lwd=1, col=2)
...
plot(women)
abline(lm(women$weight ~ women$height), col='red', lty=2, lwd=4) #~ gives the best matching line for the two parameters

#example
plot(x=mtcars$wt, y=mtcars$mpg, col=mtcars$gear, cex=mtcars$am, pch=mtcars$gear) #pch decides the type of markes, col - based on number of gear colour will change, cex-size of plotting symbol
...
text(1+.2, quantile(women$height), labels=c('min','1Q','median','3Q','max')) #1+.2 gives the label at 0.2 distance from 1st plot
#if we dont use +.2 i till label right on the centre (1st plot)
