...
tail(mtcars)
filter(mtcars, row_number() == n()) #gives the last row
filter(mtcars, between(row_number(), 5, n()-2)) #to print the rows between the two numbers



#mutate----
mutate(mtcars, displ_l = disp / 61.0237) #adds a new column after performing a partifular functions
transmute(mtcars, displ_l = disp / 61.0237) #does the same as previous just removes other columns
...
slice(by_cyl, 1:2) #groups the cars by cylinders then shows the irst two for each cylinder size.
